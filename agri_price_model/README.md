...
### Programmatic Access
You can use the prediction function directly from Python:
...
### Interactive Mode (Farmer CLI)
Run the interactive script to allow farmers to enter details and receive guidance.

```bash
python farmer_input.py
```

This presents a menu of available states, districts and commodities, then prompts for input. It will save any successful prediction to `farmer_predictions.json`.

### Command-line Query
A standalone CLI entrypoint is available via a small helper script. You can call it with arguments:

```bash
python -c "from predict import predict_best_market, print_prediction_result; result = predict_best_market('Maharashtra','Ahmednagar','Tomato'); print_prediction_result(result)"
```

For convenience you may wrap this in your own shell script or alias.
...
A ready-made script `test_enhanced_prediction.py` performs a series of example queries including fallback scenarios and writes results to the console and optionally a JSON file. Run it via:

```bash
python test_enhanced_prediction.py
```

The output confirms that every model (including fallback district/state models) responds correctly and helps you validate new datasets before deployment.
